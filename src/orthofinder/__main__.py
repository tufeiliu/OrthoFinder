...
from orthofinder import blast_file_processor, files, mcl, \
    util, matrices, orthologues, program_caller, \
    trees_msa, split_ortholog_files
...
if getattr(sys, 'frozen', False):
    ...
    if 'DYLD_LIBRARY_PATH_ORIG' in my_env: # macOS
...
def GetDirectoryArgument(arg, args):
    if len(args) == 0:
        print("Missing option for command line argument %s for %s" % (arg, get_flag_name(arg)))
        util.Fail()
...
def GetOrthologues_FromTrees(options):
    orthologues.OrthologuesFromTrees(options.recon_method, options.nBlast, \
                                     options.nProcessAlg, options.speciesTreeFN, \
                                     options.qAddSpeciesToIDs, options.qSplitParaClades, \
                                     options.fewer_open_files)
...
def ProcessesNewFasta(fastaDir, q_dna, speciesInfoObj_prev = None, speciesToUse_prev_names=[]):
    ...
    newSpeciesIDs = []
    with open(files.FileHandler.GetSequenceIDsFN(), 'a') as idsFile, \
        open(files.FileHandler.GetSpeciesIDsFN(), 'a') as speciesFile:
...
def main(args=None):    
    try:
        ...
        elif options.qStartFromFasta: # Read the fasta data from the ExampleData directory
            # 3. 
            speciesInfoObj = None
